...
function AddNewQuestion() {
  ...
  const handleCheckboxChange = (index) => {
    // Set all checkboxes to false except the one being checked
        ...
        <div className="flex items-center">
          <DndContext onDragEnd={handleDragEnd}>
            <Droppable id="droppable-container">
              <div className="w-96 mt-2 border border-lavender rounded-sm p-0.5">
                {prompts.map((prompt, index) => (
                  <Draggable key={prompt.id} id={String(prompt.id)}>
                    <div className="relative flex items-center w-full p-1 cursor-pointer">
                      <img
                        src={edit}
                        alt="Move"
                        className="w-8 mr-2.5 ml-2.5"
                      />
                      <input
                        type="text"
                        className="p-2 pr-10 border border-lavender rounded-sm bg-charcoalBlack w-full"
                        placeholder="Enter your text here"
                        value={prompt.text}
                        onChange={(e) =>
          ...
          <div className="flex flex-col items-start mr-6 mt-4">
            {prompts.map((prompt, index) => (
              <div key={prompt.id} className="flex items-center mb-2">
                <input
                  type="checkbox"
                  checked={prompt.checked}
                  onChange={() => handleCheckboxChange(index)}
                  className="appearance-none border-2 mt-3 mb-3.5 ml-2 border-white rounded-sm 
                  w-4 h-4 bg-charcoalBlack checked:bg-white 
